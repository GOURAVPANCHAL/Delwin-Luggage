...
const duffleBag = () => {
    ...
    return (
        ...
        <Container>
            <Typography variant="h3" align='center' sx={{fontSize:{xs:'30px', md:'40px', sm:'35px'}}} fontWeight={800}>
                <span className='getcolor'>D</span>uffle bags
            </Typography>
                ...
                {ProductArr.map((item) =>
                    <Grid item xs={12} md={3} >
                        <Link to={`/Products-Info/${item.name}/${item.id}`}>
                            ...
                            <img src={item.image} style={{ width: '100%', height: '100%', marginBottom:'1rem' }} alt='products' />
                            </div>
                           <Box>
                            <Typography  color='#00000099' variant="h6">{item.name}</Typography>
